...
const ProductCard = ({ product, home }) => {
    ...
    return (
        <div
            className={`my-2 w-full xs:h-[250px] md:h-[350px] flex justify-between flex-col rounded-2xl border border-slate-200 bg-white p-3 relative font-medium shadow-sm transition hover:-translate-y-1 hover:shadow-lg hover:border-slate-400`}>
            ...
            <Link to={`/product-details/${product._id}`} className='h-full flex flex-col' >

                {/* Img */}
                <div className='h-4/6 flex items-center justify-center overflow-hidden rounded-xl bg-slate-50'>
                    <img className='w-5/6 mx-auto object-contain' src={img} alt={title} />
                </div>
                {/* Title */}
